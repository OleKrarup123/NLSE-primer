...
\section{(Solitonic) Dispersive Waves}
The spatial frequency of an electric field propagating through a waveguide can be expressed as

\begin{align}
\betag_{tot}(\omega) &= \betag_{lin}(\omega)+\betag_{NL}(\omega),    
\end{align}
where $\betag_{lin}(\omega)$ is determined by the design of the waveguide and given by Eq.~\ref{eq:beta_approx}, while $\betag_{NL}(\omega)$ depends on the temporal profile of the field launched into the medium through Eq.~\ref{eq:SPM_freq}. For a general field, $\betag_{NL}(\omega)$ is tricky to calculate, but for a fundamental soliton propagating in a medium where $\betag_2<0$, $\betag_{NL}(\omega)$ exactly cancels out the impact of the term $1/2\cdot\betag_2(\omega-\omega_0)^2$ in Eq.~\ref{eq:beta_approx}. In fact, this is the very definition of a field being a soliton! Just as in Sec.~\ref{sec:Phase_matching}, where multiple CW waves having the correct combination of spatial frequencies allows for Four Wave Mixing (i.e., coherent transfer of power between them), it is possible for a soliton field centered at $\omega_0$ to be phase matched with a CW field at a different frequency. The resulting CW field is referred to as a "Dispersive Wave" (DW) in the literature, though calling it a "Solitonic Dispersive Wave" would more accurately convey that the CW field only arises in the presence of a soliton (or any pulse whose envelope is sufficiently similar to that of a soliton). In Fig.~\ref{fig:Soliton_comparison}~c), the DW field generated by the soliton becomes significant at a distance of around 0.6~m and quickly trails behind the main pulse, picking up a delay of 1000~ps after propagating from 0.6~m to 1.0~m. In Fig.~\ref{fig:Soliton_comparison}~d) it can be seen that the dispersive wave arises at a frequency of 193.43~THz. The phase matched frequency at which the dipsersive wave arises can be calculated from
...
\end{align}
Assuming that $\betag_{0,NL}\approx 0$ and that dispersion terms higher than $\betag_3$ are negligible, 
\begin{align}
...
\end{align}
Thus under these circumstances, a soliton at $\omega_0$ can coherently transfer power to a CW frequency, which is $\Delta\omega_{DW}=-3\betag_2/\betag_3$ away from it. From Eq.~\ref{eq:ZDF} the distance to the zero-dispersion-frequency in this case is $\Delta\omega_{ZD}=-\betag_2/\betag_3$, so $\Delta\omega_{DW} = 3\Delta\omega_{ZD}$. See \href{https://www.desmos.com/calculator/4zgmcegtni}{this interactive graph} for a visualization of Eq.~\ref{eq:DW}. A similar calculation for the case where only $\betag_2<0$ and $\betag_4>0$ are considered shows that two dispersive waves arise at $\Delta\omega_{DW} = \pm\sqrt{6}\Delta\omega_{ZD} = \pm \sqrt{6} \sqrt{-2\betag_2/\betag_4}\approx \pm 2.45 \sqrt{-2\betag_2/\betag_4}$. See \href{https://www.youtube.com/watch?v=EQNKfDsh2Tk}{this video tutorial} for more information on Dispersive Waves.   
